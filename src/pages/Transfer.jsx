...
const SendCrypto = () => {
  ...
  const handleEmailCodeChange = (e) => {
    setEmailCode(onlyDigits(e.target.value).slice(0, 6));
  };

  const handleAuthCodeChange = (e) => {
    setAuthError(""); // clear error while typing
    setAuthCode(onlyDigits(e.target.value).slice(0, 6));
  };

  const handleAuthPaste = (e) => {
    e.preventDefault();
  ...
  const canSubmit = emailCode.length === 6 && authCode.length === 6 && !loading;
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      // Simulate API always returns incorrect auth code
      if (!attemptedOnce) {
        setAuthError("Incorrect authentication code. Try again.");
        setAuthCode(""); // clear only the auth code
        requestAnimationFrame(() => authInputRef.current?.focus());
        setAttemptedOnce(true);
      } else {
        // Redirect to OTP page (adjust path as needed)
    ...
    <div className="bg-black min-h-screen text-white p-4 flex flex-col">
      ...
      <div className="mb-1">
        <input
          ...
          onPaste={handleAuthPaste}
          placeholder="Authentication code"
          maxLength={6}
          className={`w-full bg-gray-800 rounded-lg px-4 py-3 text-white placeholder-gray-500 ${
            authError ? "border border-red-500" : "border border-transparent"
          }`}
      ...
      {authError && (
        <p id="auth-error" className="mb-3 text-xs text-red-500">
          {authError}
        </p>
      )}
